...
//     lab= new int [100];
    ...
    for(int j=0;j<k;j++)
    {
        n=ngt[j];
        bl=1;
        //if( (n+1) > pixelnum || (n-1) < 0 || (n+Cols) > pixelnum || (n-Cols) < 0)
        //{
        //    break;
        //}
        if ( (n+1) <= pixelnum)
...
//                  dist=dist+((labelsx[lab]-x)*(labelsx[lab]-x)+(labelsy[lab]-y)*(labelsy[lab]-y))*1;
        ...
        for(int i=0;i<k;i++)
        {
            if(tlab<pixelnum && tlab>0)
            {
                label[ngt[i]]=label[tlab];
            }
        }
    }
    delete [] ngt;
...
void DBscan( unsigned char* R, unsigned char* G, unsigned char* B,int Rows,int Cols,unsigned short* label,double supnumber,int &realnumber,int post,int E, int E2)
{
    int pixel=Rows*Cols;
    ...
    for(int i=0;i<pixel;i++)
    {
        if(!label[i])
        {
            ...
            regionQuery(neighbours,numbngb, R, G, B,i,i, Rows, Cols, label, label0, E*E);// and find its neighbours
...
//                 ngbC[Nc][num]=nb;
                ...
                /*
                   if  (i<Cols&&num<10)
                   {
                       regionQuery(neighbours,numbngb,R, G, B,i,nb, Rows, Cols,label,label0,E*E);
                   }
                   else if(num<10) 
                   {
                       regionQuery(neighbours,numbngb, R, G, B,i,nb, Rows, Cols,label,label0,E*E);
        ...
        if (labnumb[n]<50 && labnumb[n]>0)
        {
           ...
           int minnum=0,mn;
            ...
            {  
                ...
                double dist1 =(dr1-R0)*(dr1-R0)+(dg1-G0)*(dg1-G0)+(db1-B0)*(db1-B0);
                dista=(x0-x1)*(x0-x1)+(y0-y1)*(y0-y1);
                double dist=dist1+dista*2;
                 if (dist<minlab)
                 {
                     minlab=dist;
                     minnum=mn;
